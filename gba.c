...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int i = 0; i < height; i++) {
    ...
    DMA[DMA_CHANNEL_3].cnt = (width | DMA_ON | DMA_DESTINATION_INCREMENT | DMA_SOURCE_FIXED);
  }
}
...
void drawFullScreenImageDMA(const u16 *image) {
  ...
  DMA[DMA_CHANNEL_3].cnt = ((WIDTH * HEIGHT) | DMA_SOURCE_INCREMENT | DMA_ON | DMA_DESTINATION_INCREMENT);
}
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for (int i = 0; i < height; i++) {
    ...
    DMA[DMA_CHANNEL_3].dst = videoBuffer + OFFSET(row + i, col, WIDTH);
    DMA[DMA_CHANNEL_3].cnt = (width | DMA_SOURCE_INCREMENT | DMA_ON | DMA_DESTINATION_INCREMENT);
  }
}
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  UNUSED(height);
  UNUSED(image);
  drawImageDMA(row, col, width, height, BLACK);
}
...
void fillScreenDMA(volatile u16 color) {
  ...
  DMA[DMA_CHANNEL_3].src = &color;
  DMA[DMA_CHANNEL_3].dst = videoBuffer;
  DMA[DMA_CHANNEL_3].cnt = ((WIDTH * HEIGHT) | DMA_SOURCE_FIXED | DMA_ON | DMA_DESTINATION_INCREMENT);
}
